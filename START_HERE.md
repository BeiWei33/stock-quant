...
如果想用真正可点击的本地 Web 控制台：

```powershell
python -m quant.apps.web
```
...
这个控制台可以点按钮运行本地流程、上传成交 CSV、查看任务日志和常用报告。它只绑定本机地址，不做实盘下单。
...
## 9. 使用 AkShare 真实 A 股行情

如果你想让系统从公开行情源获取真实 A 股日线，而不是只用本地样例数据，先安装 AkShare：
...
```powershell
python -m quant.apps.start akshare
```

默认会抓取内置的 30 只常见 A 股，时间范围约为最近 260 个自然日，并生成：
...
```powershell
python -m quant.apps.start akshare --symbols 600519.SH,000001.SZ,300750.SZ --limit 3
```

注意：AkShare 接入的是行情数据，不是交易接口。当前仍然只是研究和模拟盘流程，尚未接入 QMT 实盘下单。

## 10. AkShare 全市场回测

如果你要拉取全市场股票在指定时间段的数据，并查看收益、年化、夏普、最大回撤等指标，运行：

```powershell
python -m quant.apps.start akshare-backtest --start-date 2024-01-01 --end-date 2024-12-31
```
...
全市场会逐只股票拉取日线，第一次会比较慢。想先确认流程是否通，可以加一个小样本限制：

```powershell
python -m quant.apps.start akshare-backtest --start-date 2024-01-01 --end-date 2024-12-31 --limit 20
```

月度调仓：

```powershell
python -m quant.apps.start akshare-backtest --start-date 2024-01-01 --end-date 2024-12-31 --rebalance monthly
```
